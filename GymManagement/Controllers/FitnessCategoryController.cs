...
namespace GymManagement.Controllers
{
	...
	public class FitnessCategoryController : CognizantController
	{
        ...
        // GET: FitnessCategory
        [Authorize(Roles = "User,Client,Staff,Supervisor,Admin")]
        ...
        public async Task<IActionResult> Details(int? id)
		{
			...
			var fitnessCategory = await _context.FitnessCategories
				.Include(m => m.ExerciseCategories).ThenInclude(m => m.Exercise)
				.AsNoTracking()
		...
		public async Task<IActionResult> InsertFromExcel(IFormFile theExcel)
		{
			...
			if (theExcel != null)
			{
				...
				if (!(mimeType == "" || fileLength == 0))//File validation
				{
					if (mimeType.Contains("excel") || mimeType.Contains("spreadsheet"))
					{
						...
						int successCount = 0;
						int errorCount = 0;
						if (workSheet.Cells[1, 1].Text == "Exercise")
						{
							for (int row = start.Row + 1; row <= end.Row; row++)
							{
								...
								{
									// Row by row...
									exercise.Name = workSheet.Cells[row, 1].Text;
								...
								catch (Exception ex)
								{
									...
									{
										feedBack += "Error: Record " + exercise.Name
											+ " was rejected because it was not in the correct format." + "<br />";
									}
									else
									{
										feedBack += "Error: Record " + exercise.Name
						...
						if (workSheet.Cells[1, 2].Text == "FitnessCategory")
						{
							for (int row = start.Row + 1; row <= end.Row; row++)
							{
								...
								{
									//Row by row...
									fitnessCategory.Category = workSheet.Cells[row, 2].Text;
						...
						{
							feedBack = "Error: You may have selected the wrong file to upload.<br /> " +
								"Remember, you must have the heading 'FitnessCategory' in the " +
								"first cell of the first row.";
						}
					}
					else
